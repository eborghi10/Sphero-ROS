...
class OdometryAnalysis(object):
    ...
    def get_distance_moved(self, odometry_data_list):
        ...
        if len(odometry_data_list) >= 2 :
            ...
            end_odom = odometry_data_list[len(odometry_data_list)-1]
            ...
            rospy.logerr("Odom array doesnt have the minimum number of elements = " + str(len(odometry_data_list)))
...
def check_if_distance_reached(goal_distance, odom_result_array):
    odom_analysis_object = OdometryAnalysis()   # Creates an Odometry CLASS
    distance = odom_analysis_object.get_distance_moved(odom_result_array)
    rospy.loginfo("Distance Moved=" + str(distance))
    # To exit we consider that each square in the floor is around 0.5m, 
    # therefore to exit correctly, distance has to be sqrt (6*5 + 5*4) = 7.8
